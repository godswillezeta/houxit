...
class BaseMotion{
  ...
  function passableBlock(block, warn=false){
    try{
      ...
      if(isTrue(warn)){
        debugHandler(`[template error] invalid Statement in mustache/binding context\n\nContext expects a single expression\n\n"${block}"`);
        debugHandler(err);
      }
      return false
    }
  ...
  function compileToRenderable(value, preserve=true){
    value=unwrap(value);
    if(canRender(value)) {
      return preserve ? value : String(value);
  ...
  class HouxitEffectFrame{
    ...
    schedule(){
      if(isHouxitBuild(this.self)){
        ...
        for(let [ callback, type ] of this.callbacks.values()){
          if(type==='effect'){
            this.self[$$$operands]._OBSERVERS.add([callback, flushType]);
  ...
  function flattenWidgetAndAsyncBuild(vnode, self, is_hyperscript=false, ctx, siblings, ssc, hx_Element, config, isWidget=false){
    ...
    // if(!isSuspensible(a_p)) boundary=undefined;
...
// };
  ...
  function genericCollection(value){
    if(!validateType(value, [WeakSet, Set, Tuple, Array])){
      debugHandler("[genericCollection() Error] receives a non collection value");
      return {};
    }
    const add=validateType(value, [Set, Tuple, WeakSet]) ? 'add' : 'push';
    const has=validateType(value, [Set, Tuple, WeakSet]) ? 'has' : 'includes';
    ...
    return {
      ...
      delete(v, c=1, ...inserts){
        let args=[ v ];
        if(isArray(value)){
          args=[value.indexOf(v), c, ...inserts];
        }
  ...
  function $$dir_MODEL(self, bindings, element, hx_Element, metrics){
    ...
    if((isSSR && isString(localName)) || IS_ELEMENT_NODE(element)){
      ...
      function flushCallback(element){
        ...
        const target_value=get_I_V(element);//try converting a range/number type to a number
...
function normalize_form_model(self, element, value, modifiers, effect){
  ...
  for(let control of controls){
    ...
    if(register.has(name)){
      if(!(tag==='input' && _makeMap_('radio,checkbox', type))){//warns when more than one element poits to same [ath]
        $warn(`[Houxit $$model Warning]: Multiple form controls elements share the name "${name}" in the same form. Did you intend to use radio buttons or checkboxes instead?`, self, true);
...
function applyModelInitialState(self, element, initVal, localName, inForm){
  if(localName==='select'){
    ...
    if(_makeMap_('checkbox,radio', type)){
      ...
      if((isCollection(initVal) ? genericCollection(initVal).has(value) : !element.hasAttribute('value') ? initVal : value === initVal)){
  ...
  function serializeCSSFrames(frameCallback, samples = 100) {
    let css = '';
...
function get_Model_Event(el) {
  ...
  if (tagName === "input") {
    if(_makeMap_('checkbox,radio,date,time,datetime-local,month,week,file', el.type)){
      return 'change'
    }
    return 'input';
  }
  ...
  class Observer{
    ...
    constructor(self, deps, callback, options, EffectHook){
      ...
      if(hasOwn(options, 'flushType')){
        ...
        if(!isString(flushType) || !_makeMap_('post,sync,pre', flushType)){
          debugHandler('[unrecognised flushType] "'+flushType+'" is not a valid flushType. do you mean - post | sync | pre', self, true);
        }else {
  ...
  function _EffectDependencyNotifier(self){
    ...
    const flushType=getFlushType(self);
    ...
    return function(){
      for(let hk of postEffList.values()){
        tick(()=>callbackHookWithCatch(self, hk, "Encountered an error during an effect flush call", true));
      }
    }
  }
  ...
  class EffectFusion {
    ...
    trigger(flush) {
      ...
      this.triggered=true;
      if(flushType==='post' || flushType==='pre') {
        deferEventCircleThread(this.self, this.callback()).then(()=>{
          this.triggered=false;
        });
    ...
    if(isFunction(widgetBuild)){
      if(!isRerender && isAsyncFunction(widgetBuild)){
       ...
       return (async function(){
          ...
          const builder= await widgetBuild.call(undefined, generateBuildParams(self), getComposersContext(self));
      ...
      try{
        ...
        renderer = widgetBuild.call(undefined, generateBuildParams(self), getComposersContext(self) );
        if(useState) {
          reinstatePreviousRunningEffect();
        }
  ...
  function _tick( fn,) {
    ...
    return new Promise( ( resolve , reject ) => {
      ...
      }else if(isPFunction(fn)){
        resolve( deferEventCircleThread( self , fn , isHouxitBuild( self ) ) ) ;
      }
    } ) ;
  }
  ...
  function blockHtmlEmbedder(self, node, blockN,  metrics , [children, exp], config){
    const [ hx_Element, NodeList, tagName, context, fall ] = metrics;
    let effect = _createHouxitEffectFrame(()=>{
